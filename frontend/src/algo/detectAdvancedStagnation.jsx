...
const detectAdvancedStagnation = (exerciseName, workouts) => {
    ...
    const detectWeightStagnation = () => {
      ...
      if (uniqueWeights.length === 2) {
        ...
        if (oscillations >= 3) {
          ...
          return {
            ...
            exerciseName,
            pattern: `Oscillating between ${weight1}lbs and ${weight2}lbs`,
            message: `You've mastered the ${avgWeight}lb range! Your form and consistency are solid.`,
            confidence: `Oscillating weights shows you're right at your progression threshold - perfect timing to push forward.`,
            suggestion: `Try ${nextWeight}lbs for 3-4 reps, then work back up to your target rep range`,
      ...
      if (sameWeightCount >= 4) {
        ...
        return {
          type: 'weight_stagnation',
          exerciseName,
          pattern: `${lastWeight}lbs for ${sameWeightCount} consecutive workouts`,
          message: `You've built incredible strength endurance at ${lastWeight}lbs! Time to challenge yourself.`,
          confidence: `Consistent performance at this weight proves you're ready for the next level.`,
          suggestion: `Progress to ${nextWeight}lbs with ${Math.max(5, Math.floor(avgReps * 0.8))} reps`,
      ...
      if (sameWeightSessions.length >= 3) {
        ...
        if (isDecline && totalDecline >= 2) {
          ...
          return {
            ...
            exerciseName,
            pattern: `Reps declining: ${repTrend.join('→')} at ${currentWeight}lbs`,
            ...
            suggestion: `Deload to ${Math.round(deloadWeight)}lbs for 8-10 reps, focus on perfect form`,
    ...
    const detectFailedProgression = () => {
      ...
      if (attempts.length >= 2) {
        ...
        return {
          ...
          exerciseName,
          pattern: `Multiple attempts at ${lastAttempt.weight}lbs with reduced reps`,
          ...
          suggestion: `Try ${intermediateWeight}lbs as a stepping stone for ${lastAttempt.prevReps} reps`,
    ...
    const detectVolumePlateau = () => {
      ...
      if (Math.abs(slope) < avgVolume * 0.01) { // Less than 1% change per workout
        ...
        if (lastData.reps >= 12) {
          ...
          suggestion = `Increase weight to ${suggestedWeight}lbs and reduce reps to ${suggestedReps}`;
        } else if (lastData.reps <= 6) {
          // Low reps - add volume
          suggestedWeight = lastData.weight;
          suggestedReps = lastData.reps + 2;
          suggestion = `Keep ${suggestedWeight}lbs but increase reps to ${suggestedReps}`;
        } else {
          // Medium reps - add a set or increase weight
          suggestedWeight = lastData.weight + 5;
          suggestedReps = lastData.reps;
          suggestion = `Progress to ${suggestedWeight}lbs for ${suggestedReps} reps, or add an extra set`;
        }
        
        return {
          type: 'volume_plateau',
          exerciseName,
          pattern: `Volume plateaued around ${Math.round(avgVolume).toLocaleString()} total pounds`,
    ...
    const detectPerfectStagnation = () => {
      ...
      if (identicalCount >= 3) {
        ...
        return {
          ...
          exerciseName,
          pattern: `Identical ${lastSession.sets}×${lastSession.reps} @ ${lastSession.weight}lbs for ${identicalCount} sessions`,
          ...
          suggestion: `Time to progress: ${nextWeight}lbs for ${Math.max(lastSession.reps - 1, 5)} reps`,
  ...
  const analyzeAllExercises = (workouts) => {
    ...
    exerciseNames.forEach(exerciseName => {
      const issue = detectAdvancedStagnation(exerciseName, workouts);
      if (issue) {
  ...
  const getProgressInsights = (workouts) => {
    const stagnationIssues = analyzeAllExercises(workouts);
